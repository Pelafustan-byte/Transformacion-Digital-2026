...
function decorate(){
  ...
  document.querySelectorAll('.capCard').forEach(card=>{const label=card.querySelector('small')?.textContent||'';let host=card.querySelector('.capImage');if(!host){host=document.createElement('div');host.className='capImage fallbackArt';const image=document.createElement('img');image.src=fallbackFor(label);image.alt=`Ilustración temática: ${label.split('·')[0].trim()}`;host.append(image);card.prepend(host)}decorateImage(host.querySelector('img'),label)});
  document.querySelectorAll('.noteCard').forEach(card=>{const label=card.querySelector('small')?.textContent||'Nota institucional';let host=card.querySelector('.noteImage');if(!host){host=document.createElement('div');host.className='noteImage fallbackArt';const image=document.createElement('img');image.src='/assets/illustrations/government.svg';image.alt='Ilustración institucional de apoyo';host.append(image);card.prepend(host)}decorateImage(host.querySelector('img'),label)});
