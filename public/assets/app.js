...
async function load(){const r=await fetch('/api/public/content',{cache:'no-store'});if(!r.ok)throw new Error('content');D=await r.json();renderAll()}
...
});['territoryCaption1','territoryCaption2','territoryCaption3'].forEach(k=>text(k,s[k]));if($('brandLogoImg')&&s.brandLogo)$('brandLogoImg').src=s.brandLogo;if($('municipalCrestImg')&&s.municipalCrest)$('municipalCrestImg').src=s.municipalCrest;if(s.contactEmail){$('footerEmail').textContent=s.contactEmail;$('footerEmail').href='mailto:'+s.contactEmail}// Un fondo fotográfico solo se aplica si la fotografía aguanta el tamaño: el
...
function renderNews(){const a=D.news||[];if(!a.length){$('newsGrid').innerHTML='<div class="emptyPublic">Sin noticias publicadas.</div>';return}const lead=a[0];$('newsGrid').innerHTML=`<a class="newsFeature" href="${esc(lead.url)}" target="_blank" rel="noopener"><small>${esc(lead.tag||'Actualidad')} · ${esc(fmt(lead.publishedAt))}</small><h3>${esc(lead.title)}</h3><p>${esc(lead.description||'')}</p><b>${esc(lead.source||'Fuente oficial')} ↗</b></a><div class="newsSide">${a.slice(1,4).map(n=>`<a class="newsSmall" href="${esc(n.url)}" target="_blank" rel="noopener"><small>${esc(n.tag||'Actualidad')} · ${esc(fmt(n.publishedAt))}</small><h3>${esc(n.title)}</h3></a>`).join('')}</div>`}
