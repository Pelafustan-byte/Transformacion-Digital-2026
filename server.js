...
async function publicContent(){const c=await readContent();const vis=a=>sort((a||[]).filter(x=>!['draft','hidden'].includes(x.status)));return{site:c.site||{},videos:vis(c.videos),materials:vis(c.materials),capsules:vis(c.capsules),notes:vis(c.notes),timeline:sort(c.timeline||[]),resources:sort(c.resources||[]),news:vis(c.news),libraryCollections:sort(c.libraryCollections||[])}};
